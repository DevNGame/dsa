...
public class CountSubsetSum {
    ...
    static int getMaxValue(int[] arr, int sum){
        int n = arr.length + 1;
        int m = sum + 1;
        int[][] dp = new int[n][m]; //6 x 4 matrix
        //init
        for(int i=0; i<n; i++){
            dp[i][0] = 1;
        }
    ...
    /**
     *
     * Given an array arr[] of size n and sum, count the number of subset of the array having sum equal to sum.
     *
     ...
     * if we can, then dp[i][j] will be sum of subsets including and excluding the current element.
    ...
    public static void main(String args[]){
        int[] arr= new int[]{1, 2, 3,4, 5}; // n =5
        int sum = 3; //sum = 3
        int knapsack = getMaxValue(arr, sum);
